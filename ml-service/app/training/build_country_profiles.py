...
from app.training.country_codes import DATASET_DIR, build_name_to_iso3, normalize_name
...
def _parse_oecd_wide_table(path: Path, name_to_iso3: dict) -> pd.DataFrame:
    ...
    while i < len(rows):
        ...
        iso3 = name_to_iso3.get(normalize_name(label))
...
def build() -> pd.DataFrame:
    ...
    cost_of_living["iso3"] = cost_of_living["Country"].apply(lambda n: name_to_iso3.get(normalize_name(n)))
